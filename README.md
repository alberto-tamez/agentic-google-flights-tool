...
This command-line tool searches many route and date options from one JSON input. It preserves input order, limits concurrent requests, deduplicates identical searches, caches successful results, and reports each failed request separately.
...
The default provider opens Google Chrome in headless mode and reads the English accessibility labels in Google Flights results. If Google shows its optional consent page, the provider rejects consent and continues. The optional `fli` provider calls Google's undocumented internal endpoint. It remains available for comparison, although that endpoint returned a null response envelope during this project's testing.
