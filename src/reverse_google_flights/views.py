...
from reverse_google_flights.models import BatchReport, RankedFlight
...
def compact_summary(
    ...
    *,
    ...
    return {
        ...
        "preview": [_slim(item) for item in flattened[:preview_limit]],
        "next_actions": {
            "filter": f"reverse-google-flights list {source_ref} --filters FILTERS.json",
            "next_page": f"reverse-google-flights list {source_ref} --cursor CURSOR",
            "details": f"reverse-google-flights show {source_ref} RESULT_ID",
            "full": f"reverse-google-flights summary {source_ref} --full",
        },
    }
...
def list_page(
    ...
    *,
    ...
    return {
        ...
        "offset": offset,
        "page_size": page_size,
        "results": [_slim(item) for item in page],
        "next_cursor": next_cursor,
        "next_actions": {
            "next_page": (
                f"reverse-google-flights list {source_ref} --filters FILTERS.json "
...
def show_results(
    ...
    *,
    ...
    if not 1 <= len(result_ids) <= 10:
        raise ValueError("show requires between 1 and 10 result IDs")
...
def _slim(item: RankedFlight) -> dict[str, Any]:
    option = item.option
    return {
...
def _compact_coverage(outcome: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "fully_explored": coverage.fully_explored,
    }
